...
if __name__ == "__main__":
    ...
    file = ("CC_dtp1.0_dtn1.0_2026-05-27_12-17")
    iter = ""
    if iter != "":
    ...
    PINN.neg_model.params["SOL_n"][0] = torch.nn.Parameter(data=torch.tensor(parameters["SOL_n"][0]))
    optimizer_param_n_S = torch.optim.Adam([PINN.neg_model.params["SOL_n"][0]], lr=0.0001 * parameters["SOL_n"][0])
    ...
    for j in range(iterations + 1):
        ...
        if j % 100 == 0:
            print(j, "\t\t", V_error.detach().numpy(), "\t\t", loss_tot_n.detach().numpy(), "\t\t", loss_tot_p.detach().numpy(), "\t\t",
                  ...
                  PINN.neg_model.params["SOL_n"][0].detach().numpy(), "\t\t")
