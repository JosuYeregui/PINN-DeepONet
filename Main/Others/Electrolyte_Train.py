...
if __name__ == "__main__":
    ...
    for j in range(1000 + 1):
        ...
        if j % 1000 == 0:
            ...
            print("\t\tWeights\t\t", elec_model.weights)
    ...
    bcs_sample_x = torch.linspace(0., 1., 1000) #* elec_model.params["L"]
    bcs_sample_t = torch.zeros_like(bcs_sample_x) #* t_end
    ...
    bcs_sample_t = torch.ones_like(bcs_sample_x) #* t_end
    bcs_sample = torch.stack([bcs_sample_t, bcs_sample_x, bcs_sample_I]).t()
    ...
    plt.plot(bcs_sample_x.detach().numpy(), (ce_PINN_tend - ce_PINN_tend[0]).detach().numpy(),
             "r", label="PINN")
    plt.plot(x/elec_model.params["L"], (ce_tend - ce_tend[0]), "k", label="Pybamm")
    plt.legend()
    plt.xlabel("r [-]")
    plt.ylabel("x [-]")
    plt.show()
    ...
    plt.plot(bcs_sample_x.detach().numpy(), ce_PINN_tend.detach().numpy(),
             "r", label="PINN")
    plt.plot(x/elec_model.params["L"], ce_tend, "k", label="Pybamm")
    plt.legend()
    plt.xlabel("r [-]")
    plt.ylabel("x [-]")
    plt.show()
    ...
    t_test = (torch.arange(0, num, dtype=torch.float, requires_grad=True) / num)[::10] #* t_end
    r_rand = (torch.arange(0, num, dtype=torch.float, requires_grad=True) / num)[::10] #* parameters["L"]

    t_new, r_new = torch.meshgrid(t_test, r_rand)
    resi = np.zeros_like(t_new.detach().numpy())
    ...
    for i, (t, r) in enumerate(zip(t_new, r_new)):
        ...
        resi[i, :] = np.abs(residuals.detach().numpy())
        pos[i, :] = elec_model(torch.stack((t, r, cur)).t()).detach().numpy()
        gradients = elec_model.compute_gradients(torch.stack((t, r, cur)).t())
        pos_dcdt[i, :] = gradients[:, 0].detach().numpy()
        pos_dcdr[i, :] = gradients[:, 1].detach().numpy()
    ...
    plot_area(resi, 'RdBu_r', 'res')
    plot_area(pos, 'RdBu_r', 'pos')
