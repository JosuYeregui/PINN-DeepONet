...
class Solid_Phase(PINN):
    ...
    def __init__(self, model, parameters, criterion=nn.MSELoss(), tc=3600.,
    ...
    def _bc_surf(self, x, c, i_app):
        ...
        return ((- dcdr[:, 1] - self.electrode * np.power(self.params["R_"+self.el_name], 2) * i_app / (
                3 * self.params["eps_"+self.el_name] * self.params["D_"+self.el_name] * self.params["L_"+self.el_name]
                * self.params["F"] * self.params["A"] * self.params["c_"+self.el_name+"_max"]))
                * 0.5 * (1 + torch.tanh((x[:, 0] - 10./3600.)/(10./3600.))))
